...
CREATE TABLE IF NOT EXISTS Dog     (collarid INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, age INT, name VARCHAR(32), gender VARCHAR(1), own INT UNSIGNED, breedid INT UNSIGNED);
CREATE TABLE IF NOT EXISTS Events   (code INT UNSIGNED PRIMARY KEY AUTO_INCREMENT, note VARCHAR(256) DEFAULT '', vaccinevisit VARCHAR(8) NOT NULL DEFAULT "visit", type INT UNSIGNED, userid INT UNSIGNED NOT NULL, detailtimestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
